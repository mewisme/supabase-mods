#!/usr/bin/env bash
# Build pg_wait_sampling from pinned upstream source into $STAGING_DIR.
set -euo pipefail
...
srcdir="${work}/pg_wait_sampling-${EXT_VERSION}"
[[ -d "$srcdir" ]] || srcdir="$(find "$work" -maxdepth 1 -type d -name 'pg_wait_sampling-*' | head -n1)"
...
find "$srcdir" -maxdepth 1 -name 'pg_wait_sampling--*.sql' -exec cp -a {} "$STAGING_DIR/" \;

[[ -f "${STAGING_DIR}/pg_wait_sampling.so" ]] || die "build did not produce pg_wait_sampling.so"
[[ -f "${STAGING_DIR}/pg_wait_sampling.control" ]] || die "build did not produce pg_wait_sampling.control"
ls "${STAGING_DIR}"/pg_wait_sampling--*.sql >/dev/null 2>&1 || die "build did not produce SQL script"
